...
void moveHead(ros::Publisher& head_pub, double yaw, double pitch, double duration, const std::string& description) {
  ...
  // Use a loop with ros::Rate to allow interruption (Ctrl+C)
...
bool moveHeadDownAndUp(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res) {
  ros::NodeHandle nh;
  // Create a publisher for the head controller commands.
  ros::Publisher head_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/head_controller/command", 10);

  // Wait briefly to ensure that at least one subscriber is connected.
  ros::Rate wait_rate(10);
  double elapsed = 0.0;
  while (ros::ok() && elapsed < 1.0) {  
    wait_rate.sleep();
    elapsed += 0.1;
  }

  if (!ros::ok()) return false;  // If ROS is shutting down

  // 1) Move the head down (look down)
  moveHead(head_pub, 0.0, -1.3, 2.0, "look down");

  // 2) Pause for 2 seconds while looking down
  elapsed = 0.0;
  ...
  // 3) Move the head back up (look forward)
...
int main(int argc, char** argv) {
  ...
  // Advertise the service "/move_head_down"
